...
def _load_dataset(folder_name: str):
    ...
    df.userId = df.userId.map(dict(zip(user_ids, range(m))))
    df.movieId = df.movieId.map(dict(zip(movie_ids, range(n))))
    ...
    return (m, n), (X_train, y_train), (X_test, y_test)
...
def get_dataset(dataset) -> Tuple[TDataSet, TDataSet, TDataShape]:
    ...
    shape, *datasets = _load_dataset("ml-latest-large")
    datasets = ([
